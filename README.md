...
- **DICA SHOW:** No `it` do teste adicionar uma `template string` e o nome dos métodos de forma dinámica também, ou seja `#${UniqueIdService.prototype.generatedUniqueIdWithPrefix.name}`, assim garante que caso o nome do método mudar, será alterado no teste também;
- O `beforeEach(() => {})` dos testes faz com que, as declarações inseridas dentro dele, sejam criadas uma instância para cada condição (os `it`s) do teste, ou seja, o que estiver declarado ali nunca vai compartilhar o valor entre condições diferentes;
- A instância do artefato que será testado sempre será única, cada `it` tem que ser atômico, ele não pode sofrer influência com os dados de outros testes;
...
- O `ngInChanges` nunca será chamado com o `detectChanges` do teste, para chamá-lo é necessário fazer manualmente, com isso é preciso criar uma variável do tipo `SimpleChanges` que terá todas as modificações esperadas, nesse exemplo é um array de `photos`, esse array vai receber uma instância de `SimpleChange` passando inicialmente como estava o estado anterior, nesse caso zerado `[]`, as mudanças, aqui foi pego uma lista de photos fake `component.photos` e o terceiro parâmetro é se é a primeira change detection dessa variável, foi passado `true`. Pode-se observar esta resolução no arquivo `photo-board.component.spec.ts`, (exemplo, lá ficou `component.photos = buildPhotoList(); //código const change: SimpleChanges = { photos: new SimpleChange([], component.photos, true) }; component.ngOnChanges(change);`). Tem uma forma secundária de resolver esta situação, mas trata-se de uma gambiarra, a mesma está localizada no arquivo `photo-board-outro.component.spec.ts`;
- **DICA DO CURSO:** Não é raro o desenvolvedor aplicar lógicas de transformação diretamente nas chamadas de instâncias de HttpClient. Esse é um dos motivos que levam o desenvolvedor a utilizar o HttpClientTestingController do módulo HttpClientTestingModule para criar respostas falsas para determinadas APIs. Ele pode fornecer dados previsíveis tornando possível o teste dessas transformações.
